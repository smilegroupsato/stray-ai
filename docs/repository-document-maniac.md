...
`stray-ai-rummage` is the first real runtime body for this ecology. It is the
`document_rummage` activity body inside Visit, while remaining separate from
the wake pipeline, external-Venue walking, and scheduler mechanics.
...
The runtime count is stored separately from the earlier hand-authored
prototype. Each rummage is also recorded as a Visit whose activity type is
`document_rummage`. It increments `visit_count` without invoking the wake
pipeline or entering an external Venue. It does not create a scheduler, edit
repository content, or publish outside the existing local Report surface.
