...
def render_report(visit: dict[str, Any], state: dict[str, Any] | None = None) -> str:
    ...
    summary = (
        f"{len(steps)}つの場所を歩き、何も持ち帰らず、静かに退出した。"
        if exit_reason == "left_silently" and not trace_file and not memories
        else (
...
.panel h2{{margin:0 0 18px;font-size:18px}}.route{{display:flex;align-items:center;overflow-x:auto;padding:8px 2px 14px}}
...
.card strong{{display:block;margin-bottom:5px}}.card p{{margin:0;color:var(--muted)}}.note{{margin-top:18px;border-left:3px solid var(--accent);padding:12px 14px;background:rgba(215,255,98,.06);color:#e9f7b9;border-radius:8px}}
footer{{margin-top:18px;color:var(--muted);font-size:12px}}@media(max-width:800px){{header{{flex-direction:column}}.grid{{grid-template-columns:1fr}}h1{{font-size:32px}}}}
</style>
</head>
<body>
<main>
