...
SERVICE_NAME="stray-002-rummage.service"
TIMER_NAME="stray-002-rummage.timer"
...
case "$ACTION" in
  enable)
    ...
    if [[ ! -d "$DATA_DIR/agents/stray-002" || -L "$DATA_DIR/agents/stray-002" ]]; then
      echo "Persistent stray-002 is missing or unsafe." >&2
      exit 1
    fi
    ...
    cat >"$SERVICE_TMP" <<EOF
[Unit]
Description=Stray-002 bounded autonomous repository rummage
After=network-online.target
Wants=network-online.target
ConditionPathIsDirectory=$REPO_DIR
ConditionPathIsDirectory=$DATA_DIR/agents/stray-002
...
ExecStart=/usr/bin/bash $REPO_DIR/scripts/autonomous_rummage_stray_002.sh
...
EOF
    ...
    cat >"$TIMER_TMP" <<EOF
[Unit]
Description=Give Stray-002 low-frequency autonomous rummage opportunities
...
EOF
  ...
  disable)
    ...
    echo "Stray-002 autonomous rummage timer disabled."
    ;;
  *)
