...
cat > "$DATA_DIR/rummage-stray-002-sgos-repos-llm.sh" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec bash "$REPO_DIR/scripts/rummage_stray_002_sgos_repos_llm.sh" "\$@"
EOF
chmod 750 "$DATA_DIR/rummage-stray-002-sgos-repos-llm.sh"
...
echo "LLM Stray-002 SGOS Console desk topic rummage: $DATA_DIR/rummage-stray-002-sgos-console-llm.sh"
echo "LLM Stray-002 SGOS repository rummage: $DATA_DIR/rummage-stray-002-sgos-repos-llm.sh"
